...
func NewShoutrrr(ppfmt pp.PP, rawURLs []string) (Shoutrrr, bool) {
	...
	if err != nil {
		ppfmt.Noticef(pp.EmojiUserError, "Failed to prepare Shoutrrr notifications: %v", err)
...
func (s Shoutrrr) Send(_ context.Context, ppfmt pp.PP, msg Message) bool {
	...
	for _, err := range errs {
		if err != nil {
			ppfmt.Noticef(pp.EmojiError, "Failed to send notifications via Shoutrrr: %v", err)
			allOK = false
		}
	}
	if allOK {
