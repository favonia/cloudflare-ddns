...
// SetWAFList updates a WAF list. The IPs in detectedIPs are assumed to be non-zero.
	...
	for _, ipNet := range [...]ipnet.Type{ipnet.IP4, ipnet.IP6} {
		...
		for _, item := range items {
			if ipNet.Matches(item.Prefix.Addr()) {
				if !item.Prefix.Contains(detectedIPs[ipNet]) {
		...
		if !covered {
			itemsToCreate = append(itemsToCreate,
				netip.PrefixFrom(detectedIPs[ipNet], api.WAFListMaxBitLen[ipNet]).Masked())
		}
	}
