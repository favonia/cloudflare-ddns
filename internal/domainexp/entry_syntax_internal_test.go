...
func TestEntrySyntaxAccepts(t *testing.T) {
	...
	for input, expected := range map[string]string{
		...
		"example.org,example.net example.com":                          "missing(comma(example.org, example.net), example.com)",
		"example.org example.net,example.com":                          "missing(example.org, comma(example.net, example.com))",
...
func TestEntrySyntaxRejectSpans(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range []struct {
		...
		{input: "example.org{hostid6=[::1,,::2]}", span: syntax.Span{Start: 25, End: 26}},
		{input: "example.org{hostid6=::1,,hostid6=::2}", span: syntax.Span{Start: 24, End: 25}},
		{input: "example.org{} example.net", span: syntax.Span{Start: 14, End: 25}},
	} {
