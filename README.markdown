...
> 👉 The updater preserves existing parameters (TTL, proxy status, DNS record comments, etc.) whenever possible. During DNS reconciliation, metadata for satisfying unmatched targets is derived from stale managed records being replaced; stale records may then be recycled as an optimization of delete+create. If stale metadata is missing or ambiguous, the updater falls back to the configured values below.
