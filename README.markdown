...
| 🧪 `IP4_DEFAULT_PREFIX_LEN` (unreleased) | 🧪 The default CIDR prefix length applied to each detected IPv4 address before WAF list targets are derived from it. For example, `24` stores each detection as a `/24` range. Valid range: 8–32. | `32` |
| 🧪 `IP6_DEFAULT_PREFIX_LEN` (unreleased) | 🧪 The default CIDR prefix length applied to each detected IPv6 address before WAF list targets are derived from it. For example, `48` stores each detection as a `/48` range. Valid range: 12–128. See [IPv6 Default Prefix Length Policy](docs/designs/features/ipv6-default-prefix-length-policy.markdown) for the design rationale behind the `/64` default. | `64` |
