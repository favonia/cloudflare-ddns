...
| `url:<url>`                                                                                   | <p>Fetch the IP address from a URL. The provider format is `url:` followed by the URL itself. For example, `IP4_PROVIDER=url:https://api4.ipify.org` fetches the IPv4 address from <https://api4.ipify.org>.</p><p>The updater connects over IPv4 for `IP4_PROVIDER` and over IPv6 for `IP6_PROVIDER`. If you want to override that, use `url.via4:<url>` or `url.via6:<url>` instead.</p><p>🤖 Since version 1.15.0, plain `url:<url>` enforces the matching IP family.</p><p>Currently, only HTTP(S) is supported.</p>                                                                                                                                      |
| `url.via4:<url>` (unreleased)                                                                 | <p>Fetch the IP address from a URL while always connecting to that URL over IPv4.</p><p>Use this to get an IPv6 address over IPv4, for example with `IP6_PROVIDER=url.via4:...`.</p><p>Example: `IP6_PROVIDER=url.via4:https://example.com/my-ipv6` means the updater fetches the IPv6 address over IPv4.</p>                                                                                                                                                                               |
| `url.via6:<url>` (unreleased)                                                                 | <p>Fetch the IP address from a URL while always connecting to that URL over IPv6.</p><p>Use this to get an IPv4 address over IPv6, for example with `IP4_PROVIDER=url.via6:...`.</p><p>Example: `IP4_PROVIDER=url.via6:https://example.com/my-ipv4` means the updater fetches the IPv4 address over IPv6.</p>                                                                                                                                                                               |
