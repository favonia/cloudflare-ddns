...
| `MANAGED_RECORDS_COMMENT_REGEX` (unreleased) | Regex that selects which existing DNS records this updater manages. Only matching records are updated or deleted. Uses [RE2](https://github.com/google/re2/wiki/Syntax) syntax (not Perl/PCRE).                                                         | `""` (matches all comments) |
...
| `WAF_LIST_ITEM_COMMENT` (unreleased)                | Comment added to new WAF list items created by this updater.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | `""`                        |
| `MANAGED_WAF_LIST_ITEMS_COMMENT_REGEX` (unreleased) | Regex that selects which existing WAF list items this updater manages. Uses [RE2](https://github.com/google/re2/wiki/Syntax) syntax (not Perl/PCRE). The empty value matches all comments. With `DELETE_ON_STOP=true`, a non-empty regex prevents whole-list deletion and limits shutdown cleanup to matched items.                                                                                                                                                                                                                                                                                                                 | `""` (matches all comments) |
...
| `DELETE_ON_STOP`   | Whether managed DNS records and managed WAF content are deleted when the updater exits. It accepts any boolean value supported by [strconv.ParseBool](https://pkg.go.dev/strconv#ParseBool), such as `true`, `false`, `0`, or `1`. For WAF lists, cleanup scope depends on `MANAGED_WAF_LIST_ITEMS_COMMENT_REGEX`: with a non-empty selector, shutdown cleanup deletes only matched managed items and keeps the list; with the default empty selector, all list items are managed, so the updater may try deleting each configured list (or clear items when list deletion is blocked by a rule expression).                                      | `false`                       |
