...
>
> The final DNS address set for a domain is the deduplicated cross-product of all detected IPv6 prefixes and the domain's effective `hostid6` set. Repeated declarations for the same domain must specify exactly the same `hostid6` set; omitting `hostid6` from a partial declaration expresses no opinion. If any configured host ID is incompatible with any detected prefix, the updater preserves all existing IPv6 DNS records and IPv6 WAF list items for that update. IPv4 continues independently. `hostid6` changes only DNS targets; WAF lists still use the detected IPv6 prefixes directly.
>
