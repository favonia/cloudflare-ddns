...
| 🧪 `WAF_LIST_ITEM_COMMENT` (unreleased)                | 🧪 Default comment for newly created WAF list items. When replacing stale managed items, the updater may inherit the stale comment if it is unambiguous; otherwise it falls back to this value.                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | `""`                                           |
| 🧪 `MANAGED_WAF_LIST_ITEMS_COMMENT_REGEX` (unreleased) | 🧪 Regex that matches comments of existing WAF list items this updater manages. Only items whose comments match are managed by this updater and may be deleted during reconciliation or shutdown cleanup. Cloudflare does not provide an API to edit a single WAF list item in place. During reconciliation, when the desired IP/range set changes, the updater adds missing items and removes stale items; newly created item comments are inherited from stale managed items when unambiguous, and otherwise fall back to `WAF_LIST_ITEM_COMMENT`. Uses [RE2](https://github.com/google/re2/wiki/Syntax) syntax (not Perl/PCRE). With `DELETE_ON_STOP=true`, a non-empty regex prevents whole-list deletion and limits shutdown cleanup to matched items. | `""` (empty regex; manages all WAF list items) |
...
> 👉 The updater preserves existing parameters (TTL, proxy status, DNS record comments, etc.) whenever possible. DNS records are usually recycled, so existing DNS metadata is often preserved in place. When creating new DNS records or WAF list items during reconciliation, the updater first tries to inherit metadata from stale managed objects being replaced. If stale metadata is missing or ambiguous, it falls back to the configured values below.

| Name                                             | Meaning                                                                                                                                                                                                                                                                                                   | Default Value                              |
| ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------ |
| `PROXIED`                                        | <p>Default proxy setting for newly created DNS records. When stale managed records being replaced have a unanimous proxy setting, that setting is inherited; otherwise this value is used. It can be any boolean value accepted by [strconv.ParseBool](https://pkg.go.dev/strconv#ParseBool), such as `true`, `false`, `0` or `1`.</p><p>🤖 Advanced usage: it can also be a domain-dependent boolean expression as described below.</p> | `false`                                    |
| `TTL`                                            | Default TTL (in seconds) for newly created DNS records. When stale managed records being replaced have a unanimous TTL, that value is inherited; otherwise this value is used.                                                                                                                                                                                 | `1` (This means “automatic” to Cloudflare) |
| `RECORD_COMMENT`                                 | Default [record comment](https://developers.cloudflare.com/dns/manage-dns-records/reference/record-attributes/) for newly created DNS records. When stale managed records being replaced have a unanimous comment, that comment is inherited; otherwise this value is used.                                                                                                                                      | `""`                                       |
| 🧪 `WAF_LIST_DESCRIPTION` (since version 1.14.0) | 🧪 The text description of new WAF lists.                                                                                                                                                                                                                                                                 | `""`                                       |
| 🧪 `WAF_LIST_ITEM_COMMENT` (unreleased)          | 🧪 Default comment for newly created WAF list items. Existing items are not rewritten in place. When stale managed items being replaced have a unanimous comment, that comment is inherited; otherwise this value is used.                                                                                                                                                                                          | `""`                                       |
