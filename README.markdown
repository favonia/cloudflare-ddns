...
> The updater can maintain [WAF lists](https://developers.cloudflare.com/waf/tools/lists/custom-lists/) to match detected IP addresses, with new IPv6 detections stored as `/64` ranges by default. 🤖 Existing ranges in the list that already cover a detected address are kept as-is.
>
> 🧪 Both defaults (IPv4 addresses as individual addresses and IPv6 ones as `/64` ranges) are configurable via `IP4_DEFAULT_PREFIX_LEN` and `IP6_DEFAULT_PREFIX_LEN` (unreleased). 🤖 See [IPv6 Default Prefix Length Policy](docs/designs/features/ipv6-default-prefix-length-policy.markdown) for the design rationale.
...
| 🧪 `IP4_DEFAULT_PREFIX_LEN` (unreleased) | 🧪 The default CIDR prefix length attached to each detected IPv4 bare address. This affects how WAF lists store IPv4 detection results. For example, `24` will store each detection as a `/24` range. Valid range: 8–32. The maximum value `32` (default) means storing each address as individual ones.                                                                                                                                                                       | `32`               |
| 🧪 `IP6_DEFAULT_PREFIX_LEN` (unreleased) | 🧪 The default CIDR prefix length attached to each detected IPv6 bare address. This affects how WAF lists store IPv6 detection results. For example, `48` stores each detection as a `/48` range. Valid range: 12–128. The maximum value `128` means storing each address as individual ones. See [IPv6 Default Prefix Length Policy](docs/designs/features/ipv6-default-prefix-length-policy.markdown) for the design rationale behind the `/64` default (instead of `/128`). | `64`               |
