...
| `MANAGED_RECORDS_COMMENT_REGEX` (unreleased) | A regular expression used to select which existing DNS records are managed by this updater instance. Only matched records are updated/deleted. The syntax is [RE2](https://github.com/google/re2/wiki/Syntax) (not Perl/PCRE).                        | `""` (matches all comments) |
...
| `WAF_LIST_ITEM_COMMENT` (unreleased)                | The comment written to newly created WAF list items for this updater instance.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `""`                        |
| `MANAGED_WAF_LIST_ITEMS_COMMENT_REGEX` (unreleased) | A regular expression used to select which existing WAF list items are managed by this updater instance. The syntax is [RE2](https://github.com/google/re2/wiki/Syntax) (not Perl/PCRE), and the empty regex naturally matches all comments. With `DELETE_ON_STOP=true`, a non-empty regex prevents whole-list deletion and scopes shutdown cleanup to matched items.                                                                                                                                                                                                                                                                                        | `""` (matches all comments) |

> 🤖 **Advanced setup for shared-domain and shared-WAF-list multi-instance deployments**
>
> Use this setup when multiple updater instances may manage overlapping DNS domains or share one or more WAF lists and you want each instance to manage only its own WAF list items.
>
> 1. Give each instance a unique `RECORD_COMMENT`.
> 2. Set `MANAGED_RECORDS_COMMENT_REGEX` to the same DNS comment using `^...$`.
> 3. If instances may touch the same WAF list, give each instance a unique `WAF_LIST_ITEM_COMMENT`.
> 4. Set `MANAGED_WAF_LIST_ITEMS_COMMENT_REGEX` to the same WAF item comment using `^...$`.
>
> Example:
>
> - Instance A: `RECORD_COMMENT=managed-by-ddns-a`, `MANAGED_RECORDS_COMMENT_REGEX=^managed-by-ddns-a$`, `WAF_LIST_ITEM_COMMENT=managed-by-ddns-a`, and `MANAGED_WAF_LIST_ITEMS_COMMENT_REGEX=^managed-by-ddns-a$`
> - Instance B: `RECORD_COMMENT=managed-by-ddns-b`, `MANAGED_RECORDS_COMMENT_REGEX=^managed-by-ddns-b$`, `WAF_LIST_ITEM_COMMENT=managed-by-ddns-b`, and `MANAGED_WAF_LIST_ITEMS_COMMENT_REGEX=^managed-by-ddns-b$`
>
...
>
> `DELETE_ON_STOP=true` always deletes managed DNS records. For WAF lists, setting a non-empty `MANAGED_WAF_LIST_ITEMS_COMMENT_REGEX` prevents whole-list deletion and limits shutdown cleanup to items managed by this updater.
...
> 2. The updater will delete IP addresses belonging to disabled IP families from WAF list items managed by this updater (_e.g.,_ if you disable IPv6 with `IP6_PROVIDER=none`, then existing IPv6 addresses or IPv6 ranges in managed items will be deleted). The managed set is selected by `MANAGED_WAF_LIST_ITEMS_COMMENT_REGEX`.
...
| `DELETE_ON_STOP`   | Whether managed DNS records and managed WAF content should be deleted on exit. It can be any boolean value accepted by [strconv.ParseBool](https://pkg.go.dev/strconv#ParseBool), such as `true`, `false`, `0` or `1`. For WAF lists, cleanup scope follows `MANAGED_WAF_LIST_ITEMS_COMMENT_REGEX`: with a non-empty selector, shutdown cleanup deletes only matched managed items and does not delete the list itself; with the default empty selector, all list items are managed, and the updater may try deleting each configured list (or clear that list when deletion is blocked by a rule expression).                                                                                                 | `false`                       |
...
> 👉 The updater will preserve existing parameters (TTL, proxy statuses, DNS record comments, etc.). Only when it creates new DNS records, new WAF lists, and new WAF list items, the following settings will apply: newly created WAF list items use `WAF_LIST_ITEM_COMMENT`, and ownership of existing WAF list items is selected by `MANAGED_WAF_LIST_ITEMS_COMMENT_REGEX`. To change existing parameters, you can go to your [Cloudflare Dashboard](https://dash.cloudflare.com) and change them directly. If you think you have a use case where the updater should actively overwrite existing parameters in addition to IP addresses, please [let me know](https://github.com/favonia/cloudflare-ddns/issues/new).
...
| `INTERFACE=<iface>`                    | ✔️  | To automatically select the local address, use `IP4/6_PROVIDER=local`. 🧪 To select all global unicast addresses of a specific network interface, use `IP4/6_PROVIDER=local.iface:<name>` (available since version 1.15.0; experimental and not finalized yet). The updater reconciles DNS records and WAF lists against the full detected set. |
