...
Detection filters are experimental; their expression syntax may change. A filter expression can use `keep-all`, `addr-in(<CIDR>)`, `!`, `&&`, `||`, and parentheses:
...
`addr-in(<CIDR>)` checks only the detected address, not its prefix length. Use explicit CIDR notation: `/32` for a single IPv4 address or `/128` for a single IPv6 address.
