...
- 🛡️ The updater uses only secure protocols ([DNS over HTTPS (DoH)](https://en.wikipedia.org/wiki/DNS_over_HTTPS) and HTTPS) to detect public IP addresses, which makes it harder to tamper with the detection process. _(Due to the nature of public IP address detection, it is impossible to defend the updater against a strong advisory that can modify the source IP address of the IP packages coming from your machine without disturbing TCP connections to remote servers.)_
