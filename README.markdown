...
> 💡 Active cleanup tip: set one or both IP providers to `static.empty` and use `UPDATE_CRON=@once` to remove managed DNS records or managed WAF items and then exit. Add `DELETE_ON_STOP=true` to make the updater try to delete the WAF list itself too.
