...
| `IP4_DETECTION_FILTER` (unreleased)                       | Filter detected IPv4 entries after provider normalization and before DNS records and WAF lists are derived. A filter uses a boolean expression such as `addr-in(198.51.100.0/24)`. `addr-in(P)` requires explicit CIDR notation, checks only the address part of each detected entry, and leaves detected prefix lengths unchanged for downstream DNS and WAF derivation. If the filter removes every IPv4 entry, IPv4 aborts for that round and existing managed IPv4 resources are preserved.                                                                                              | `keep-all`         |
| `IP6_DETECTION_FILTER` (unreleased)                       | Filter detected IPv6 entries after provider normalization and before DNS records and WAF lists are derived. A filter uses a boolean expression such as `!addr-in(fc00::/7)`. `addr-in(P)` requires explicit CIDR notation, checks only the address part of each detected entry, and leaves detected prefix lengths unchanged for downstream DNS and WAF derivation. If the filter removes every IPv6 entry, IPv6 aborts for that round and existing managed IPv6 resources are preserved.                                                                                                    | `keep-all`         |
...
environment:
  ...
  - IP4_DETECTION_FILTER=addr-in(198.51.100.0/24) || addr-in(203.0.113.7/32)
```

`addr-in(P)` requires explicit CIDR notation. For single hosts, write `/32` for IPv4 or `/128` for IPv6.
