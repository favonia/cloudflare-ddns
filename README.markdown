...
> 👉 The updater will preserve existing proxy and TTL settings and other record attributes until it has to create new DNS records (or recreate deleted ones). Only when it creates DNS records, the following settings will apply. To change existing proxy and TTL settings or other record attributes now, you can go to your [Cloudflare Dashboard](https://dash.cloudflare.com) and change them directly. If you think you have a use case where the updater should actively overwrite existing proxy and TTL settings or other attributes in addition to IP addresses, please [let me know](https://github.com/favonia/cloudflare-ddns/issues/new).
