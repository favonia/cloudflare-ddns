...
| 🧪 `SHOUTRRR`  | 🧪 A list of notifications services the updater should send messages to when it updates IP addresses. The format is newline-separated [shoutrrr URLs](https://containrrr.dev/shoutrrr/), such as `discord://<token>@<id>`.                                                                                                                                                                                                                                                                                                                                                                                                                                             |

> ⚠️ Please note that a failure when handling IPv6 would cause the status to be reported as _down_ even if IPv4 records are updated successfully (and similarly if IPv6 works but IPv4 fails). If your network does not support IPv6, add `IP6_PROVIDER=none` to disable IPv6 completely.
